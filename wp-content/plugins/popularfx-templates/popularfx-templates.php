...
function pfx_load_license(){
	...
	if(empty($popularfx['license'])){
		$popularfx['license'] = array();
	}
	
	$last_update = isset($popularfx['license']['last_update']) ? $popularfx['license']['last_update'] : 0;
	
	// Update license details as well
	if( !empty($popularfx['license']) && (time() - $last_update) >= 86400 ){
		
		$resp = wp_remote_get(pfx_api_url().'license.php?license='.$popularfx['license']['license'].'&url='.rawurlencode(site_url()));
	...
	if(empty($popularfx['license']) || empty($popularfx['license']['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			if(!empty($softaculous_license['license']) && 
				(!empty($softaculous_license['active']) || empty($popularfx['license']['license']))
			){
				$popularfx['license'] = $softaculous_license;
			}
		}
	}
